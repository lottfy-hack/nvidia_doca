 ...
 */
static doca_error_t daemon_am_recv_data_complete_callback(void *arg, ucs_status_t status)
{
	...
	if (allreduce_super_request->num_allreduce_requests == allreduce_config.num_clients) {
		...
		DOCA_LOG_WARN(
			"More vectors than clients were received for a single Allreduce operation. Ignoring vector of new client (considered duplicates) but including in the final result response");
	} else {
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t daemon_am_recv_ctrl_callback(struct allreduce_ucx_am_desc *am_desc)
{
	...
	return allreduce_ucx_am_recv(am_desc,
				     allreduce_request->vector,
				     length,
				     daemon_am_recv_data_complete_callback,
				     allreduce_request,
				     NULL);
}
...
{
	struct sigaction new_sigaction = {.sa_handler = signal_terminate_handler, .sa_flags = 0};

 ...
 */
static inline void user_request_complete(allreduce_ucx_callback callback,
					 void *arg,
 ...
 */
static doca_error_t request_process(struct allreduce_ucx_connection *connection,
				    enum allreduce_ucx_op op,
				    ucs_status_ptr_t ptr_status,
				    allreduce_ucx_callback callback,
				    void *arg,
				    int log_err,
				    struct allreduce_ucx_request **request_p)
{
 ...
 */
static inline void common_request_callback(void *request, ucs_status_t status, void *user_data, enum allreduce_ucx_op op)
{
	struct allreduce_ucx_request *r = (struct allreduce_ucx_request *)request;
...
int allreduce_ucx_request_wait(int ret, struct allreduce_ucx_request *request)
{
	if (ret < 0 && request != NULL) {
		...
		DOCA_LOG_ERR("The operation %p failed: %s",
			     allreduce_ucx_op_str[request->op],
		...
		if (request->status != UCS_OK) {
			...
			if (request->log_err) {
				...
				DOCA_LOG_ERR("The operation %s failed: %s",
					     allreduce_ucx_op_str[request->op],
					     ucs_status_string(request->status));
			}
			ret = -1;
 ...
 */
static doca_error_t am_send(struct allreduce_ucx_connection *connection,
			    unsigned int am_id,
			    int log_err,
			    const void *header,
...
doca_error_t allreduce_ucx_am_send(struct allreduce_ucx_connection *connection,
				   unsigned int am_id,
				   const void *header,
				   size_t header_length,
				   const void *buffer,
...
doca_error_t allreduce_ucx_am_recv(struct allreduce_ucx_am_desc *am_desc,
				   void *buffer,
				   size_t length,
				   allreduce_ucx_callback callback,
				   void *arg,
				   struct allreduce_ucx_request **request_p)
{
	...
	ucp_request_param_t param = {/* Completion callback and user data are specified */
...
void allreduce_ucx_am_desc_query(struct allreduce_ucx_am_desc *am_desc,
...
{
	ucp_am_handler_param_t param = {/* AM identifier, callback and argument are set */
...
void allreduce_ucx_am_set_recv_handler(struct allreduce_ucx_context *context,
				       unsigned int am_id,
 ...
 * @return: UCS_OK
 */
static ucs_status_t am_connection_check_recv_callback(void *arg,
						      const void *header,
						      size_t header_length,
						      void *data,
						      size_t length,
 ...
 */
static void disconnect_common(struct allreduce_ucx_connection *connection, uint32_t flags)
{
	struct allreduce_ucx_context *context = connection->context;
 ...
 */
static inline void connection_deallocate(struct allreduce_ucx_connection *connection)
{
 ...
 */
static struct allreduce_ucx_connection *connection_allocate(struct allreduce_ucx_context *context,
 ...
 */
static int connect_common(struct allreduce_ucx_context *context,
			  ucp_ep_params_t *ep_params,
 ...
 * @return: 0 on success and negative value otherwise
 */
static int sockaddr_connect(struct allreduce_ucx_context *context,
			    const struct sockaddr_storage *dst_saddr,
			    struct allreduce_ucx_connection **connection_p)
{
	ucp_ep_params_t ep_params = {/* Flags and socket address are specified */
 ...
 */
static void sockaddr_str(const struct sockaddr *saddr, size_t buf_len, char *buf)
{
	uint16_t port;
...
int allreduce_ucx_connect(struct allreduce_ucx_context *context,
...
static void connect_callback(ucp_conn_request_h conn_req, void *arg)
{
	...
	ucp_ep_params_t ep_params = {/* Connection request is specified */
				     .field_mask = UCP_EP_PARAM_FIELD_CONN_REQUEST,
				     /* Connection request */
				     .conn_request = conn_req};
	ucp_conn_request_attr_t conn_req_attr = {/* Request getting the client's address which is an initiator of the
						    connection */
	...
	if (status == UCS_OK) {
		...
		DOCA_LOG_ERR("Got new connection request %p, connection request query failed: %s",
			     conn_req,
...
{
	ucp_params_t context_params = {/* Features, request initialize callback and request size are specified */
				       .field_mask = UCP_PARAM_FIELD_FEATURES | UCP_PARAM_FIELD_REQUEST_INIT |
	...
	ucp_worker_params_t worker_params = {/* Thread mode is specified */
					     .field_mask = UCP_WORKER_PARAM_FIELD_THREAD_MODE,
					     /* UCP worker progress and all send/receive operations must be called from
					      * a single thread at the same time
					      */
...
int allreduce_ucx_listen(struct allreduce_ucx_context *context, uint16_t port)
{
	...
	const struct sockaddr_in listen_addr = {/* Set IPv4 address family */
						.sin_family = AF_INET,
						.sin_addr =
							{/* Set any address */
							 .s_addr = INADDR_ANY},
						...
						.sin_port = htons(port)};
	ucp_listener_params_t listener_params = {/* Socket address and conenction handler are specified */

 ...
 */
static doca_error_t set_role_param(void *param, void *config)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_allreduce_mode_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
	const char *str = (const char *)param;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_dest_ip_str_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_dest_port_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_listen_port_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_num_clients_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_size_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_datatype_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
	const char *str = (const char *)param;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_operation_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
	const char *str = (const char *)param;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_batch_size_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_num_batches_param(void *param, void *config)
{
	struct allreduce_config *app_config = (struct allreduce_config *)config;
...
doca_error_t register_allreduce_params(void)
{
	...
	doca_argp_param_set_description(allreduce_mode_param,
					"Set allreduce mode: \"offloaded\", \"non-offloaded\" (valid for client only)");
	...
	doca_argp_param_set_description(
		num_clients_param,
		"Set the number of clients which participate in allreduce operations (valid for daemon only)");
	...
	doca_argp_param_set_description(
		datatype_param,
		"Set datatype (\"byte\", \"int\", \"float\", \"double\") of vector elements to do allreduce for");
	...
	doca_argp_param_set_description(operation_param,
					"Set operation (\"sum\", \"prod\") to do between allreduce vectors");
	...
	doca_argp_param_set_description(
		dest_ip_str_param,
		"Set comma-separated list of destination IPv4/IPv6 addresses and ports optionally (<ip_addr>:[<port>]) of daemons or clients");
 ...
 */
static void allreduce_super_request_destroy(struct allreduce_super_request *allreduce_super_request)
{
	...
	if (allreduce_super_request->num_allreduce_requests > 0) {
		STAILQ_FOREACH_SAFE(current, &allreduce_super_request->allreduce_requests_list, entry, next)
		{
			...
			STAILQ_REMOVE(&allreduce_super_request->allreduce_requests_list,
				      current,
				      allreduce_request,
				      entry);
 ...
 */
static void allreduce_super_request_destroy_callback(gpointer data)
{
 ...
 */
static struct allreduce_super_request *allreduce_super_request_allocate(const struct allreduce_header *header,
									size_t length,
									void *result_vector)
{
							    ...
							    void *result_vector)
{
	...
	if (allreduce_super_request == NULL) {
		if (allreduce_config.role == ALLREDUCE_CLIENT)
			DOCA_LOG_DBG("Starting new operation with id %zu, for vector size: %zu",
				     header->id,
				     result_length);
		/* If there is no allreduce super request in the hash, allocate it */
		allreduce_super_request = allreduce_super_request_allocate(header, result_length, result_vector);
		if (allreduce_super_request == NULL)
			return NULL;

		/* Insert the allocated allreduce super request to the hash */
		g_hash_table_insert(allreduce_super_requests_hash,
				    &allreduce_super_request->header.id,
...
struct allreduce_request *allreduce_request_allocate(struct allreduce_ucx_connection *connection,
						     const struct allreduce_header *header,
						     size_t length)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t daemon_allreduce_complete_client_operation_callback(void *arg, ucs_status_t status)
{
	(void)status;
 ...
 */
static doca_error_t allreduce_complete_operation_callback(void *arg, ucs_status_t status)
{
	...
	STAILQ_FOREACH_SAFE(allreduce_request,
			    &allreduce_super_request->allreduce_requests_list,
			    entry,
			    tmp_allreduce_request)
	{
		...
		STAILQ_REMOVE(&allreduce_super_request->allreduce_requests_list,
			      ...
			      allreduce_request,
			      entry);
		allreduce_ucx_am_send(allreduce_request->connection,
				      ALLREDUCE_CTRL_AM_ID,
				      &allreduce_super_request->header,
				      ...
				      daemon_allreduce_complete_client_operation_callback,
				      allreduce_request,
				      NULL);
	}
	return DOCA_SUCCESS;
...
void allreduce_scatter(struct allreduce_super_request *allreduce_super_request)
{
	...
	for (i = 0; i < allreduce_config.dest_addresses.num; ++i)
		allreduce_ucx_am_send(connections[i],
				      ALLREDUCE_OP_AM_ID,
				      &allreduce_super_request->header,
				      sizeof(allreduce_super_request->header),
				      allreduce_super_request->result_vector,
				      allreduce_super_request->result_vector_size *
					      allreduce_datatype_size[allreduce_config.datatype],
				      allreduce_complete_operation_callback,
				      allreduce_super_request,
				      NULL);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t _allreduce_gather_callback(void *arg, ucs_status_t status)
{
	struct allreduce_super_request *allreduce_super_request = arg;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t allreduce_gather_callback(struct allreduce_ucx_am_desc *am_desc)
{
	struct allreduce_ucx_connection *connection;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t allreduce_incoming_handshake_send_callback(void *arg, ucs_status_t status)
{
	free(arg);
	if (status != UCS_OK) {
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t allreduce_incoming_handshake_recv_callback(void *arg, ucs_status_t status)
{
	...
	handshake_msg_len = snprintf(handshake_msg,
				     HANDSHAKE_MAX_MSG_LEN,
				     HANDSHAKE_MSG_FMT,
				     allreduce_config.vector_size,
				     allreduce_datatype_str[allreduce_config.datatype],
				     allreduce_config.batch_size,
	...
	if (recv_handshake->peer_role == ALLREDUCE_CLIENT && allreduce_config.role == ALLREDUCE_DAEMON) {
		...
		allreduce_ucx_am_send(connection,
				      ALLREDUCE_HANDSHAKE_AM_ID,
				      &reply_header,
				      1,
				      handshake_msg_arg,
				      handshake_msg_len,
				      allreduce_incoming_handshake_send_callback,
				      handshake_msg_arg,
				      NULL);
	}

	/* Warn user */
	DOCA_LOG_ERR("Configuration mismatch. Us: \"%s\", Other: \"%s\"", handshake_msg, recv_handshake->msg);
	if (allreduce_config.role == ALLREDUCE_CLIENT && allreduce_config.allreduce_mode == ALLREDUCE_OFFLOAD_MODE) {
		free(recv_handshake);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t allreduce_incoming_handshake_callback(struct allreduce_ucx_am_desc *am_desc)
{
	...
	return allreduce_ucx_am_recv(am_desc,
				     handshake_arg->msg,
				     length,
				     allreduce_incoming_handshake_recv_callback,
				     handshake_arg,
				     NULL);
}
 ...
 */
static int allreduce_outgoing_handshake(int num_connections)
{
	int i;
	doca_error_t result;
	...
	int handshake_msg_len = snprintf(handshake_msg,
					 HANDSHAKE_MAX_MSG_LEN,
					 HANDSHAKE_MSG_FMT,
					 allreduce_config.vector_size,
					 allreduce_datatype_str[allreduce_config.datatype],
	...
	for (i = 0; i < num_connections; ++i) {
		result = allreduce_ucx_am_send(connections[i],
					       ALLREDUCE_HANDSHAKE_AM_ID,
					       &header,
					       1,
					       handshake_msg,
 ...
 * @num_connections [in]: Number of peers/daemon
 */
static void communication_destroy(int num_connections)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t communication_init(int *nb_connections)
{
	int result;
	int num_connections;

	/* Allocate hash of allreduce requests to hold submitted operations */
	allreduce_super_requests_hash =
		g_hash_table_new_full(g_size_t_hash, g_size_t_equal, NULL, allreduce_super_request_destroy_callback);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t dest_addresses_init(void)
{
 ...
 */
static void stream_desc(void *stream_p)
{
 ...
 */
static void allreduce_destroy_mempools(void)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t allreduce_init_mempools(void)
{
	doca_error_t result;
#ifdef GPU_SUPPORT
	...
	result = allreduce_create_super_reqs_pool(2 * allreduce_config.batch_size,
						  sizeof(struct allreduce_super_request),
						  CPU);
	...
	result |= allreduce_create_vecs_pool(
		...
		allreduce_config.vector_size * allreduce_datatype_size[allreduce_config.datatype],
		vecs_mtype);
	/*
	 * First received vector from a peer will be the peers_result_vector, if there are more peers we need
	 * a buffer to hold other peers vectors. If there is only <=1 peers, the pool isn't initialized.
	...
	if (allreduce_config.dest_addresses.num > 1) {
		result |=
			allreduce_create_bufs_pool(2 * allreduce_config.batch_size,
						   allreduce_config.dest_addresses.num *
	...
	if (allreduce_config.role == ALLREDUCE_DAEMON) {
		result |= allreduce_create_reqs_pool(2 * allreduce_config.batch_size * allreduce_config.num_clients,
						     sizeof(struct allreduce_request),
						     CPU);
	}
#ifdef GPU_SUPPORT

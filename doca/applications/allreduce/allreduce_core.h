...
#define STAILQ_FOREACH_SAFE(_node, _list, _name, _temp_node) \
	for (_node = STAILQ_FIRST(_list), _temp_node = ((_node) != NULL) ? STAILQ_NEXT(_node, _name) : NULL; \
	     (_node) != NULL; \
	     _node = _temp_node, _temp_node = ((_node) != NULL) ? STAILQ_NEXT(_node, _name) : NULL)
...
struct allreduce_super_request {
	STAILQ_HEAD(allreduce_requests, allreduce_request)
	allreduce_requests_list;	 /* List of allreduce requests

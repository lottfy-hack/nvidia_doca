 ...
 */
static doca_error_t _populate_mem_pool_range(struct mpool *mpool, size_t start, size_t end)
{
	size_t i = start;
	...
	case CUSTOM:
		...
		break;
	default:
		DOCA_LOG_ERR("Memory allocation failed. Unknown type: %d (should be from 'enum mem_type')",
			     mpool->mtype);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static inline doca_error_t allreduce_create_mpool(struct mpool *mp, size_t nb_elems, size_t elem_size, mem_type mtype)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static inline doca_error_t allreduce_destroy_mpool(struct mpool *mp, const char *mp_name)
{
	size_t i = mp->head;
 ...
 */
static inline doca_error_t allreduce_aloc_vec(struct mpool *mp, void **vec_p)
{
 ...
 */
static inline void allreduce_free_vec(struct mpool *mp, void *vec)
{
	--mp->head;
 ...
 */
static inline void allreduce_free_vecs(struct mpool *mp, void **vecs, size_t n)
{
	size_t i;

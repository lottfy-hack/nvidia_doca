 ...
 */
static void allreduce_vectors_cleanup(size_t num_allreduce_vectors)
{
	size_t vector_iter;
 ...
 */
static void allreduce_vectors_reset(void)
{
 ...
 */
static doca_error_t allreduce_vectors_init(void)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t client_am_recv_data_complete_callback(void *arg, ucs_status_t status)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t client_am_recv_ctrl_callback(struct allreduce_ucx_am_desc *am_desc)
{
	...
	return allreduce_ucx_am_recv(am_desc,
				     ...
				     length,
				     client_am_recv_data_complete_callback,
 ...
 */
static void allreduce_metrics_reset(struct allreduce_metrics *allreduce_metrics)
{
 ...
 */
static void allreduce_metrics_calculate(double run_time,
					double compute_time,
					struct allreduce_metrics *allreduce_metrics)
{
	...
	/* Average run time */
	allreduce_metrics->runtime_avg = allreduce_metrics->runtime_total / (allreduce_metrics->current_batch_iter + 1);
 ...
 */
static void cpu_exploit(struct allreduce_metrics *allreduce_metrics)
{
	/*
	 * Small matrix is used because the network time is could be small and we want to have a computation time close
	 * to the network time
	 */
	...
	if (allreduce_metrics->compute_time < 0.) {
		...
		allreduce_metrics->compute_repeats =
			MAX(1, (int)((num_reps * allreduce_metrics->network_time) / estimated_overall_time));
 ...
 */
static void allreduce_metrics_print(struct allreduce_metrics *allreduce_metrics)
{
	DOCA_LOG_INFO(
		"Allreduce (%s/%s/%s) and matrix multiplication metrics to complete %zu batches (batch size - %zu, vector size - %zu):",
		allreduce_metrics->mode_str,
		allreduce_metrics->operation_str,
		allreduce_metrics->datatype_str,
		allreduce_metrics->current_batch_iter,
		allreduce_metrics->batch_size,
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t allreduce_offloaded_complete_ctrl_send_callback(void *arg, ucs_status_t status)
{
	(void)arg;
 ...
 */
static void allreduce_offloaded_submit(struct allreduce_super_request *allreduce_super_request)
{
	...
	allreduce_ucx_am_send(connection,
			      ...
			      allreduce_offloaded_complete_ctrl_send_callback,
			      NULL,
			      NULL);
}
 ...
 * @allreduce_super_request [in]: The allreduce operation context
 */
static void allreduce_non_offloaded_submit(struct allreduce_super_request *allreduce_super_request)
{
 ...
 */
static size_t allreduce_batch_submit(size_t vector_size, size_t batch_size, allreduce_submit_func submit_func)
{
	...
	for (op_iter = 0; op_iter < batch_size; ++op_iter) {
		...
		allreduce_super_request = allreduce_super_request_get(
			&allreduce_header,
			vector_size,
 ...
 */
static void allreduce_batch_wait(void)
{
 ...
 * @submit_func [in]: Pointer to a method that scatters a single Allreduce operation
 */
static void allreduce_barrier(allreduce_submit_func submit_func)
{
 ...
 * @submit_func [in]: Pointer to a method that scatters a single Allreduce operation
 */
static void allreduce_metrics_init(struct allreduce_metrics *allreduce_metrics, allreduce_submit_func submit_func)
{
 ...
 * @submit_func [in]: Pointer to a method that scatters a single Allreduce operation
 */
static void allreduce(allreduce_submit_func submit_func)
{
	struct allreduce_metrics allreduce_metrics;
	...
	for (allreduce_metrics.current_batch_iter = 0;
		...
		DOCA_LOG_TRC(
			"%zu: current run time - %.3f seconds, compute - %.3f seconds, min - %.3f seconds, max - %.3f seconds, avg - %.3f seconds",
			allreduce_metrics.current_batch_iter,
			run_time,
			compute_time,
			allreduce_metrics.runtime_min,
			allreduce_metrics.runtime_max,
 ...
 */
static void allreduce_vectors_cleanup_wrapper(void)
{
...
void client_run(void)
{
	...
	case ALLREDUCE_OFFLOAD_MODE:
		/* Setup receive handler for Active message control messages from daemon which carry the allreduce
		 * result */

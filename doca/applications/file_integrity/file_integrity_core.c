 ...
 */
static doca_error_t set_endpoint_properties(enum file_integrity_mode mode,
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t receive_and_calculate_partial_sha(struct doca_comm_channel_ep_t *ep,
						      ...
						      int timeout)
{
	...
	for (i = 0; i < total_msgs; i++) {
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("doca_buf_set_data() for request doca_buf failure: %s",
				     doca_error_get_descr(result));
			goto free_task;
		}
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t send_file(struct doca_dev *dev,
			      struct doca_comm_channel_ep_t *ep,
			      struct doca_comm_channel_addr_t **peer_addr,
			      char *file_data,
			      size_t file_size)
{
	...
	result = doca_sha_cap_get_partial_hash_block_size(doca_dev_as_devinfo(dev),
							  SHA_ALGORITHM,
				   ...
				   struct doca_sha *sha_ctx)
{
	...
	while ((result = doca_comm_channel_ep_sendto(ep, sha_msg, hash_length, DOCA_CC_MSG_FLAG_NONE, *peer_addr)) ==
	       DOCA_ERROR_AGAIN)
		nanosleep(&ts, &ts);
				   ...
				   struct doca_sha *sha_ctx)
{
	...
	msg_len = MAX_MSG_SIZE;
	while ((result = doca_comm_channel_ep_recvfrom(ep, received_sha, &msg_len, DOCA_CC_MSG_FLAG_NONE, peer_addr)) ==
	       DOCA_ERROR_AGAIN) {
		nanosleep(&ts, &ts);
	}
	if (result != DOCA_SUCCESS) {
	...
	msg_len = MAX_MSG_SIZE;
	counter = 0;
	while ((result = doca_comm_channel_ep_recvfrom(ep, received_msg, &msg_len, DOCA_CC_MSG_FLAG_NONE, peer_addr)) ==
	       DOCA_ERROR_AGAIN) {
		msg_len = MAX_MSG_SIZE;
		nanosleep(&ts, &ts);
		counter++;
	...
	if (total_msgs == 1) {
		...
		result = receive_and_calculate_partial_sha(ep,
							   peer_addr,
							   state,
							   sha_ctx,
							   &dst_doca_buf,
...
finish_msg:
	...
	while ((tmp_result = doca_comm_channel_ep_sendto(ep,
							 finish_msg,
							 sizeof(finish_msg),
							 DOCA_CC_MSG_FLAG_NONE,
							 *peer_addr)) == DOCA_ERROR_AGAIN)
				 ...
				 struct doca_sha **sha_ctx)
{
	...
	if (app_cfg->mode == SERVER) {
		result = open_doca_device_rep_with_pci(cc_doca_dev,
	...
	if (app_cfg->mode == CLIENT) {
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Couldn't establish a connection with the server node: %s",
				     doca_error_get_descr(result));
			goto destroy_core_objs;
		}

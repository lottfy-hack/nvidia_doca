...
#define BE_IPV4_ADDR(a, b, c, d) \
	(RTE_BE32(((a) << 24) + ((b) << 16) + ((c) << 8) + (d))) /* Convert IPv4 address to big endian */
 ...
 */
static doca_error_t nat_mode_callback(void *param, void *config)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t lan_intf_callback(void *param, void *config)
{
	struct nat_cfg *nat_cfg = (struct nat_cfg *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t wan_intf_callback(void *param, void *config)
{
	struct nat_cfg *nat_cfg = (struct nat_cfg *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t nat_rules_callback(void *param, void *config)
{
	struct nat_cfg *nat_cfg = (struct nat_cfg *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t nat_args_validation_callback(void *config)
{
	struct nat_cfg *nat_cfg = (struct nat_cfg *)config;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_local_ip(struct json_object *cur_rule, struct nat_rule_match *rule)
{
	doca_error_t result;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_global_ip(struct json_object *cur_rule, struct nat_rule_match *rule)
{
	doca_error_t result;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_local_port(struct json_object *cur_rule, struct nat_rule_match *rule)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_global_port(struct json_object *cur_rule, struct nat_rule_match *rule)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t nat_port_create(uint8_t portid, struct doca_flow_port **port)
{
 ...
 */
static void nat_stop_ports(int nb_ports)
{
	int portid;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_static_mode_rules(struct json_object *parsed_json,
					     int *nat_num_rules,
					     struct nat_rule_match **nat_rules)
{
	int i;
	doca_error_t result;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_pat_mode_rules(struct json_object *parsed_json,
					  int *nat_num_rules,
					  struct nat_rule_match **nat_rules)
{
	int i;
	...
	if (result != DOCA_SUCCESS)
		return result;
	DOCA_LOG_DBG("PAT global IP = %d.%d.%d.%d",
		     (parsed_global_ip & 0xff),
		     (parsed_global_ip >> 8 & 0xff),
		     (parsed_global_ip >> 16 & 0xff),
		     (parsed_global_ip >> 24 & 0xff));
...
doca_error_t parsing_nat_rules(char *file_path,
			       enum nat_mode mode,
			       int *nat_num_rules,
			       struct nat_rule_match **nat_rules)
{
...
doca_error_t nat_init(struct nat_cfg *app_cfg, struct application_dpdk_config *app_dpdk_config)
{
	...
	uint16_t nb_ports;
	struct doca_flow_cfg *nat_flow_cfg;
	...
	result = doca_flow_cfg_create(&nat_flow_cfg);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create doca_flow_cfg: %s", doca_error_get_descr(result));
		return result;
	}

	result = doca_flow_cfg_set_pipe_queues(nat_flow_cfg, app_dpdk_config->port_config.nb_queues);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to set doca_flow_cfg pipe_queues: %s", doca_error_get_descr(result));
		doca_flow_cfg_destroy(nat_flow_cfg);
		return result;
	}

	result = doca_flow_cfg_set_mode_args(nat_flow_cfg, "vnf,hws");
	if (result != DOCA_SUCCESS) {
	...
	result = doca_flow_cfg_set_cb_entry_process(nat_flow_cfg, check_for_valid_entry);
	if (result != DOCA_SUCCESS) {
	...
	result = doca_flow_cfg_set_queue_depth(nat_flow_cfg, QUEUE_DEPTH);
	if (result != DOCA_SUCCESS) {
	...
	linear_array_init_u16(rss_queues, app_dpdk_config->port_config.nb_queues);
	rss.queues_array = rss_queues;
	result = doca_flow_cfg_set_default_rss(nat_flow_cfg, &rss);
	if (result != DOCA_SUCCESS) {
	...
	result = doca_flow_cfg_destroy(nat_flow_cfg);
	if (result != DOCA_SUCCESS) {
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t build_static_local_pipe(uint16_t port_id, struct nat_rule_match *nat_rules, int nat_num_rules)
{
	...
	result = doca_flow_pipe_cfg_set_name(pipe_cfg, "NAT_STATIC_PIPE");
	if (result != DOCA_SUCCESS) {
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t build_static_global_pipe(uint16_t port_id, struct nat_rule_match *nat_rules, int nat_num_rules)
{
	struct doca_flow_match match;
	struct doca_flow_fwd fwd;
	struct doca_flow_fwd miss_fwd;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t build_pat_local_pipe(uint16_t port_id, struct nat_rule_match *nat_rules, int nat_num_rules)
{
	struct doca_flow_match match;
	struct doca_flow_fwd fwd;
	struct doca_flow_fwd miss_fwd;
	...
	result = doca_flow_pipe_cfg_set_name(pipe_cfg, "NAT_PAT_PIPE");
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to set doca_flow_pipe_cfg name: %s", doca_error_get_descr(result));
		goto destroy_pipe_cfg;
	}
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t build_pat_global_pipe(uint16_t port_id, struct nat_rule_match *nat_rules, int nat_num_rules)
{
	struct doca_flow_match match;
	struct doca_flow_fwd fwd;
	struct doca_flow_fwd miss_fwd;
	...
	for (portid = 0; portid < nb_ports; portid++) {
		...
		case STATIC:
			if (dev_info.switch_info.name != NULL &&
				...
				DOCA_LOG_ERR(
					"Getting interface index (%d) which isn't match to any configured port: %s",
					portid,
					strerror(-ret));
				return DOCA_ERROR_INVALID_VALUE;
			}

...
static bool query_pipe_info(struct security_gateway_pipe_info *pipe)
{
	...
	for (i = 0; i < pipe->nb_entries; i++) {
		result = doca_flow_query_entry(pipe->entries_info[i].entry, &query_stats);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to query entry: %s", doca_error_get_descr(result));
			continue;
		}
		if (query_stats.total_pkts != pipe->entries_info[i].prev_stats) {
			delta = query_stats.total_pkts - pipe->entries_info[i].prev_stats;
			if (first) {
				printf("%s[%s:%ld", pipe->name, pipe->entries_info[i].name, delta);
				first = false;
			} else
				printf(",%s:%ld", pipe->entries_info[i].name, delta);
			pipe->entries_info[i].prev_stats = query_stats.total_pkts;
		}
	}
	if (!first)
		printf("] ");
	...
	for (i = 0; i < NUM_OF_SYNDROMES; i++) {
		...
		if (query_stats.total_pkts != decrypt_rule->entries[i].previous_stats) {
			...
			DOCA_LOG_INFO("Got bad syndrome: %s, number of hits since last dump: %ld",
				      syndrome_list[i],
				      query_stats.total_pkts - decrypt_rule->entries[i].previous_stats);
...
static void query_decrypt_pipes(struct ipsec_security_gw_config *app_cfg)
{
	...
	changed |= query_pipe_info(&app_cfg->decrypt_pipes.decap_ipv4_pipe);
	changed |= query_pipe_info(&app_cfg->decrypt_pipes.decap_ipv6_pipe);
	if (changed)
		printf("\n");
}
				   ...
				   int decrypt_array_size)
{
	...
	for (entry_idx = 0; entry_idx < decrypt_array_size; entry_idx++) {
		decrypt_rules[entry_idx].antireplay_state.window_size = WINDOW_SIZE;
		decrypt_rules[entry_idx].antireplay_state.end_win_sn =
			(uint32_t)(app_cfg->sn_initial) + WINDOW_SIZE - 1;
...
/*
 * Check if insertion rate meaure is enabled
 *
 * @app_cfg [in]: application configuration struct

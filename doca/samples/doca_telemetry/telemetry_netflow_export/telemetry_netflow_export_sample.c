...
static doca_error_t init_template_fields(void)
{
	...
	result |=
		add_netflow_field(DOCA_TELEMETRY_NETFLOW_INPUT_SNMP, DOCA_TELEMETRY_NETFLOW_INPUT_SNMP_DEFAULT_LENGTH);
	result |= add_netflow_field(DOCA_TELEMETRY_NETFLOW_OUTPUT_SNMP,
				    DOCA_TELEMETRY_NETFLOW_OUTPUT_SNMP_DEFAULT_LENGTH);
	result |= add_netflow_field(DOCA_TELEMETRY_NETFLOW_L4_SRC_PORT,
				    DOCA_TELEMETRY_NETFLOW_L4_SRC_PORT_DEFAULT_LENGTH);
	result |= add_netflow_field(DOCA_TELEMETRY_NETFLOW_L4_DST_PORT,
				    DOCA_TELEMETRY_NETFLOW_L4_DST_PORT_DEFAULT_LENGTH);
 ...
 */
static void prepare_netflow_example_record(struct doca_telemetry_netflow_example_record *record)
{
...
doca_error_t telemetry_netflow_export(void)
{
	...
	struct doca_telemetry_netflow_example_record *records[DOCA_TELEMETRY_NETFLOW_EXAMPLE_EVENTS_BATCH_SIZE];
	...
	for (i = 0; i < DOCA_TELEMETRY_NETFLOW_EXAMPLE_EVENTS_NOF_BATCHES; i++) {
		...
		result = doca_telemetry_netflow_send(example_template,
						     (const void **)&records,

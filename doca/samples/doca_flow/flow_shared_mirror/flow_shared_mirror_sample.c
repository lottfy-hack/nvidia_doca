...
doca_error_t flow_shared_mirror(int nb_ports, int nb_queues, struct flow_switch_ctx *ctx)
{
	...
	result = doca_flow_shared_resource_cfg(DOCA_FLOW_SHARED_RESOURCE_MIRROR, shared_mirror_ids[0], &cfg);
	if (result != DOCA_SUCCESS) {
	...
	result = doca_flow_shared_resource_cfg(DOCA_FLOW_SHARED_RESOURCE_MIRROR, shared_mirror_ids[1], &cfg);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to cfg shared mirror");
		stop_doca_flow_ports(nb_ports, ports);
		doca_flow_destroy();

 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
static doca_error_t create_udp_pipe(struct doca_flow_port *port,
				    ...
				    struct doca_flow_pipe **pipe)
{
	...
	result = set_flow_pipe_cfg(cfg, "UDP_PIPE", DOCA_FLOW_PIPE_BASIC, true);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to set doca_flow_pipe_cfg: %s", doca_error_get_descr(result));
		goto destroy_pipe_cfg;
	}
					     ...
					     uint16_t ct_queue)
{
	...
	for (i = 0; i < entries_arr_len; i++) {
		result =
			doca_flow_ct_query_entry(ct_queue, NULL, flags, entries_arr[i], &query_o, &query_r, &last_hit_s);
		if (result != DOCA_SUCCESS) {

...
/*
 * Create DOCA Flow pipe that match NVGRE traffic with changeable vs_id/flow_id and do hairpin action
 *
 * @port [in]: port of the pipe
 * @port_id [in]: port ID of the pipe
 * @pipe [out]: created pipe pointer
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
static doca_error_t create_nvgre_pipe(struct doca_flow_port *port, int port_id, struct doca_flow_pipe **pipe)
{
	struct doca_flow_match match;
	struct doca_flow_actions actions, *actions_arr[NB_ACTIONS_ARR];
	struct doca_flow_fwd fwd;
...
/*
 * Add DOCA Flow pipe entry with example NVGRE vs_id/flow_id to match
 *
 * @pipe [in]: pipe of the entry
 * @status [in]: user context for adding entry
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
static doca_error_t add_nvgre_pipe_entry(struct doca_flow_pipe *pipe, struct entries_status *status)
{
	struct doca_flow_match match;
	struct doca_flow_actions actions;
	struct doca_flow_pipe_entry *entry;
	...
	return doca_flow_pipe_add_entry(0, pipe, &match, &actions, NULL, NULL, DOCA_FLOW_NO_WAIT, status, &entry);
}
...
doca_error_t flow_control_pipe(int nb_queues)
{
	...
	for (port_id = 0; port_id < nb_ports; port_id++, num_of_entries = 0) {
		...
		result = doca_flow_shared_resource_cfg(DOCA_FLOW_SHARED_RESOURCE_COUNTER, port_id, &cfg);
		if (result != DOCA_SUCCESS) {
		...
		result = create_nvgre_pipe(ports[port_id], port_id, &nvgre_pipe);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to add gre pipe: %s", doca_error_get_descr(result));
			stop_doca_flow_ports(nb_ports, ports);
			doca_flow_destroy();
		...
		result = add_nvgre_pipe_entry(nvgre_pipe, &status);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to add gre pipe entry: %s", doca_error_get_descr(result));
			stop_doca_flow_ports(nb_ports, ports);
			doca_flow_destroy();

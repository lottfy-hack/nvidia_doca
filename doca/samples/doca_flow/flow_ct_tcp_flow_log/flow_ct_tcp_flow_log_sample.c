 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise.
 */
static doca_error_t create_tcp_pipe(struct doca_flow_port *port,
				    struct doca_flow_pipe *fwd_pipe,
				    struct entries_status *status,
				    struct doca_flow_pipe **pipe)
{
	...
	result = set_flow_pipe_cfg(cfg, "TCP_PIPE", DOCA_FLOW_PIPE_BASIC, true);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to set doca_flow_pipe_cfg: %s", doca_error_get_descr(result));
		goto destroy_pipe_cfg;
	}
...
static void flow_log_cb(struct doca_flow_pipe *pipe, void *entry, uint16_t ct_queue, void *usr_ctx)
{
	...
	result = doca_flow_ct_get_entry(ct_queue, NULL, 0, entry, &match_o, &match_r, &entry_flags);
	if (result != DOCA_SUCCESS) {
	...
	result = doca_flow_ct_query_entry(ct_queue, NULL, 0, entry, &query_o, &query_r, &last_hit_s);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to query CT entry: %s", doca_error_get_descr(result));
		return;
	}

	DOCA_LOG_INFO("Entry %d, origin total bytes: %ld", nb_flow_log_triggers, query_o.total_bytes);
	DOCA_LOG_INFO("Entry %d, origin total packets: %ld", nb_flow_log_triggers, query_o.total_pkts);
...
doca_error_t flow_ct_tcp_flow_log(uint16_t nb_queues, struct doca_dev *ct_dev)
{
	...
	result = init_doca_flow_ct(ct_dev,
				   ct_flags,
				   nb_arm_queues,
				   nb_ctrl_queues,
				   nb_user_actions,

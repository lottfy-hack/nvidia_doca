...
static void flr_event_handler_cb(struct doca_devemu_pci_dev *pci_dev, union doca_data user_data)
{
	...
	DOCA_LOG_INFO("FLR has occured destroying PCI device and recreating it");
				      ...
				      uint32_t db_id)
{
	...
	if (db_id >= (uint32_t)max_num_db) {
		devemu_resources_cleanup(&resources, destroy_rep);
		DOCA_LOG_ERR(
			"DB ID must not excceed the maximum number of DBs per PCI type: expected DB ID less than %u but received %u",
			max_num_db,
			db_id);
		return DOCA_ERROR_NOT_SUPPORTED;
	}
	...
	if (resources.error != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Some error occured during progress: %s", doca_error_get_descr(resources.error));
		devemu_resources_cleanup(&resources, destroy_rep);

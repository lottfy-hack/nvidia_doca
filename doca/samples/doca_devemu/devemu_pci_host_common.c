				   ...
				   struct bar_mapped_region *mapped_mem)
{
	...
	if (bar_region_config->start_address > reg.size ||
		...
		DOCA_LOG_ERR(
			"The provided stateful region excceeds the boundaries of the emulated device's bar. At bar %d, the size is %lluB, but requested to map region with start address %zu, and size of %zuB",

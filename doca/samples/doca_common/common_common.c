...
doca_error_t sync_event_cc_handshake(struct sync_event_runtime_objects *se_rt_objs)
{
	...
	char recv;
	size_t sz = SYNC_EVENT_CC_ACK_SIZE;
	...
	while ((result = doca_comm_channel_ep_recvfrom(se_rt_objs->ep,
						       &recv,
						       &sz,
						       DOCA_CC_MSG_FLAG_NONE,
						       &se_rt_objs->peer_addr)) == DOCA_ERROR_AGAIN) {
		if (timeout == 0) {
			DOCA_LOG_ERR("Failed to receive ack from host: timeout");
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to receive ack from host: %s", doca_error_get_descr(result));
		return result;
	}
	DOCA_LOG_INFO("Received ack from host");
...
doca_error_t sync_event_cc_handshake(struct sync_event_runtime_objects *se_rt_objs)
{
	doca_error_t result = DOCA_SUCCESS;
	int timeout = SYNC_EVENT_CC_TIMEOUT_SEC;
	char send = SYNC_EVENT_CC_ACK_VALUE;
	...
	while ((result = doca_comm_channel_peer_addr_update_info(se_rt_objs->peer_addr)) ==
	...
	DOCA_LOG_INFO("Sending ack to DPU");
	while ((result = doca_comm_channel_ep_sendto(se_rt_objs->ep,
						     &send,
						     SYNC_EVENT_CC_ACK_SIZE,
						     DOCA_CC_MSG_FLAG_NONE,
						     se_rt_objs->peer_addr)) == DOCA_ERROR_AGAIN) {
		if (timeout == 0) {
			DOCA_LOG_ERR("Failed to send ack to DPU: timeout");
			return DOCA_ERROR_TIME_OUT;
		}
		sleep(1);
		timeout--;
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to send ack to DPU: %s", doca_error_get_descr(result));
		return result;
	}
	DOCA_LOG_INFO("Ack to DPU was sent");

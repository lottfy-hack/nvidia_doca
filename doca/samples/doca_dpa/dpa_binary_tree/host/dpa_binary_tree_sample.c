...
static doca_error_t create_dpa_events(struct doca_dpa *doca_dpa,
				      ...
				      struct events_export_handle *wait_events,
				      struct events_export_handle *comp_events)
{
	doca_error_t result;
	int i, j;
	...
	for (i = 0; i < NUM_OF_EVENTS; i++) {
		...
		if (i == 0) {
			...
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed to create node sync event %d: %s",
					     i,
		...
		result = doca_sync_event_get_dpa_handle(wait_events->events[i],
							doca_dpa,
							&(wait_events->event_handles[i]));
		if (result != DOCA_SUCCESS) {
		...
		result = doca_sync_event_get_dpa_handle(comp_events->events[i],
							doca_dpa,
							&(comp_events->event_handles[i]));
		if (result != DOCA_SUCCESS) {
...
doca_error_t binary_tree(struct dpa_resources *resources)
{
	...
	result = create_dpa_events(resources->doca_dpa,
				   ...
				   &wait_events_export_handle,
				   &comp_events_export_handle);
	if (result != DOCA_SUCCESS) {
	...
	for (i = 0; i < NUM_OF_NODES; i++) {
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to wait for host completion event %d: %s",
				     i,
				     doca_error_get_descr(result));
			goto destroy_events;
		}

...
static doca_error_t create_dpa_events(struct doca_dpa *doca_dpa,
				      ...
				      struct doca_sync_event **node_events,
				      doca_dpa_dev_sync_event_t *node_event_handles,
				      ...
				      doca_dpa_dev_sync_event_t *node_d_comp_event_handle)
{
	...
	for (i = 0; i < NUM_OF_NODES; i++) {
		...
		result = doca_sync_event_get_dpa_handle(node_events[i], doca_dpa, &(node_event_handles[i]));
		if (result != DOCA_SUCCESS) {
	...
	result = doca_sync_event_get_dpa_handle(*node_d_comp_event, doca_dpa, node_d_comp_event_handle);
	if (result != DOCA_SUCCESS) {
...
doca_error_t diamond_tree(struct dpa_resources *resources)
{
	...
	result = doca_dpa_kernel_launch_update_set(resources->doca_dpa,
						   ...
						   NULL,
						   0,
						   node_event_handles[1],
						   node_event_handles[2]);
	if (result != DOCA_SUCCESS) {
	...
	for (i = 1; i < NUM_OF_NODES - 1; i++) {
		...
		result = doca_dpa_kernel_launch_update_set(resources->doca_dpa,
							   ...
							   num_dpa_threads,
							   &diamond_kernel,
							   node_event_handles[i],
							   one_parent_wait_thresh,
							   node_event_handles[NUM_OF_NODES - 1],
							   NULL);
		if (result != DOCA_SUCCESS) {
...
destroy_events:
	...
	for (i = 0; i < NUM_OF_NODES; i++) {
		...
		if (tmp_result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to destroy Kernel %c wait event: %s",
				     'A' + i,

...
struct thread_arguments {
	...
	doca_dpa_dev_buf_arr_t remote_buf_arr_dpa_handle; /* DPA handle of buf array */
 ...
 */
static doca_error_t update_thread_event(struct doca_dpa *doca_dpa,
					struct doca_sync_event *kernel_comp_event,
					uint64_t comp_count,
					doca_dpa_dev_sync_event_t thread_event_handler,
					uint64_t thread_event_val)
{
	doca_error_t result;

	result = doca_dpa_kernel_launch_update_set(doca_dpa,
						   NULL,
						   0,
						   kernel_comp_event,
						   comp_count,
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_dpa_events(struct doca_dpa *doca_dpa,
				      ...
				      doca_dpa_dev_sync_event_t *thread_event_handler)
{
	...
	result = export_doca_remote_net_sync_event_to_dpa(doca_device,
							  doca_dpa,
							  *put_signal_comp_event,
							  remote_put_signal_comp_event,
							  remote_put_signal_comp_event_handle);
	if (result != DOCA_SUCCESS) {
	...
	result = doca_sync_event_get_dpa_handle(*thread_event, doca_dpa, thread_event_handler);
	if (result != DOCA_SUCCESS) {
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_rdma_resources(struct doca_dpa *doca_dpa,
					  struct doca_dev *doca_device,
					  unsigned int rdma_caps,
					  struct doca_rdma **rdma,
					  doca_dpa_dev_rdma_t *rdma_dpa_handle,
					  const void **rdma_connection_details,
					  size_t *rdma_connection_details_len)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_mmap_resources(struct doca_dev *doca_device,
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t create_buf_array_resources(struct doca_dpa *doca_dpa,
					       struct doca_mmap *mmap,
					       size_t element_size,
					       ...
					       doca_dpa_dev_buf_arr_t *dpa_buf_arr)
{
	...
	result = doca_buf_arr_create(num_elements, buf_arr);
	if (result != DOCA_SUCCESS) {
	...
	result = doca_buf_arr_set_params(*buf_arr, mmap, element_size, 0);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add DOCA device: %s", doca_error_get_descr(result));
...
void *remote_rdma_thread_func(void *args)
{
	...
	unsigned int access = DOCA_ACCESS_FLAG_LOCAL_READ_WRITE | DOCA_ACCESS_FLAG_RDMA_WRITE |
	...
	*(thread_args->remote_buff) = REMOTE_BUF_VALUE;
	...
	result = create_doca_dpa_completion_sync_event(thread_args->resources->doca_dpa,
						       thread_args->resources->doca_device,
						       &kernel_comp_event);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create kernel_comp_event: %s", doca_error_get_descr(result));
		goto free_buffer;
	...
	result = create_rdma_resources(thread_args->resources->doca_dpa,
				       thread_args->resources->doca_device,
				       access,
				       (struct doca_rdma **)&remote_rdma,
				       NULL,
				       (const void **)&(thread_args->remote_connection_details),
				       &(thread_args->remote_connection_details_len));
	if (result != DOCA_SUCCESS) {
	...
	result = create_mmap_resources(thread_args->resources->doca_device,
				       access,
				       thread_args->remote_buff,
				       buffer_size,
				       (struct doca_mmap **)&remote_buf_mmap);
	if (result != DOCA_SUCCESS) {
	...
	result = create_buf_array_resources(thread_args->resources->doca_dpa,
					    remote_buf_mmap,
					    buffer_size,
					    1,
					    (struct doca_buf_arr **)&buf_array,
					    &(thread_args->remote_buf_arr_dpa_handle));
	if (result != DOCA_SUCCESS) {
	 ...
	 * all the resources
	 */
	result = update_thread_event(thread_args->resources->doca_dpa,
				     kernel_comp_event,
				     comp_event_val++,
				     thread_args->thread_event_handler,
				     thread_event_val++);
	if (result != DOCA_SUCCESS) {
	...
	result = doca_rdma_connect(remote_rdma,
				   thread_args->local_connection_details,
	...
	DOCA_LOG_INFO(
		"Remote thread finished connecting to the main thread's rdma instances, signaling to main thread");
...
doca_error_t dpa_rdma_copy(struct dpa_resources *resources)
{
	...
	doca_dpa_dev_rdma_t local_rdma_handle;
	...
	doca_dpa_dev_buf_arr_t local_buf_arr_dpa_handle;
	...
	result = create_dpa_events(resources->doca_dpa,
	...
	args.thread_event = thread_event, args.thread_event_handler = thread_event_handler,
	...
	/* Create DOCA rdma and its resources */
	result = create_rdma_resources(resources->doca_dpa,
				       resources->doca_device,
				       access,
				       (struct doca_rdma **)&local_rdma,
				       &local_rdma_handle,
				       (const void **)&(args.local_connection_details),
	...
	result = create_mmap_resources(resources->doca_device,
				       access,
				       &local_buff,
				       sizeof(local_buff),
				       (struct doca_mmap **)&local_buf_mmap);
	...
	result = create_buf_array_resources(resources->doca_dpa,
					    local_buf_mmap,
					    sizeof(local_buff),
					    1,
					    (struct doca_buf_arr **)&buf_array,
					    &local_buf_arr_dpa_handle);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create DOCA buf array resources: %s", doca_error_get_descr(result));
		goto destroy_mmap;
	}
	 ...
	 * all the resources
	 */
	result = update_thread_event(resources->doca_dpa,
				     kernel_comp_event,
				     comp_event_val++,
				     thread_event_handler,
				     thread_event_val++);
	...
	result = doca_dpa_kernel_launch_update_set(resources->doca_dpa,
						   ...
						   0,
						   num_dpa_threads,
						   &dpa_rdma_write_and_signal,
						   (uint64_t)local_rdma_handle,
						   (uint64_t)(args.remote_buf_arr_dpa_handle),
						   (uint64_t)local_buf_arr_dpa_handle,
						   remote_put_signal_comp_event_handle,
						   put_signal_comp_event_val);
	if (result != DOCA_SUCCESS) {

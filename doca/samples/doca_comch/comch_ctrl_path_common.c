					 ...
					 struct doca_pe **pe)
{
	...
	result = doca_comch_client_set_max_msg_size(*client, CC_MAX_MSG_SIZE);
	if (result != DOCA_SUCCESS) {
					 ...
					 struct doca_pe **pe)
{
	...
	result = doca_comch_server_event_connection_register(*server,
	...
	result = doca_comch_server_set_max_msg_size(*server, CC_MAX_MSG_SIZE);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to set msg size property with error = %s", doca_error_get_name(result));
		goto destroy_server;
	}
